...
PLATFORM = "naver"
BASE_URL = "https://search.shopping.naver.com/search/all?query={query}&sort=rel"
...
async def crawl_query(page: Page, query: str, selectors: dict) -> list[dict]:
    ...
    try:
        ...
        item_selector = selectors.get("item", "div.product_item__MDtDF")
        # networkidle 대신 domcontentloaded + 명시적 셀렉터 대기
        try:
            await page.wait_for_selector(item_selector, timeout=12000)
        except PlaywrightTimeout:
            # 대체 셀렉터 시도
            item_selector = "div[class*='product_item']"
            await page.wait_for_selector(item_selector, timeout=8000)
        items = await page.query_selector_all(item_selector)

        for item in items[:5]:
            try:
                name_el = await item.query_selector(selectors.get("name", "a.product_link__TrAac"))
                price_el = await item.query_selector(selectors.get("price", "span.price_num__S2p_v"))
                link_el = await item.query_selector(selectors.get("link", "a.product_link__TrAac"))
                seller_el = await item.query_selector(selectors.get("seller", "span.product_mall_name__Hk3_g"))

                name = await name_el.inner_text() if name_el else ""
                price_text = await price_el.inner_text() if price_el else "0"
                href = await link_el.get_attribute("href") if link_el else ""
                seller = await seller_el.inner_text() if seller_el else "네이버 쇼핑"
                ...
                if name and price > 0:
                    products.append({
                        ...
                        "platform": PLATFORM,
                        "seller": seller.strip(),
                        "is_official": "공식" in name or "공식" in seller,
                        "url": href,
        ...
        print(f"[WARN] 네이버 타임아웃: {query}", file=sys.stderr)
    except Exception as e:
        print(f"[WARN] 네이버 크롤링 오류 ({query}): {e}", file=sys.stderr)
...
def main():
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"[WARN] 네이버 크롤링 실패 (시도 {attempt}): {e}", file=sys.stderr)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    out_path = OUTPUT_DIR / "raw_naver_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"platform": PLATFORM, "products": products}, f, ensure_ascii=False, indent=2)

    print(f"[OK] 네이버 크롤링 완료: {len(products)}개 상품")
