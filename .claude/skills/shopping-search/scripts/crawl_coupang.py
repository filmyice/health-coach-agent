...
PLATFORM = "coupang"
BASE_URL = "https://www.coupang.com/np/search?q={query}&channel=user&listSize=10"
...
async def crawl_query(page: Page, query: str, selectors: dict) -> list[dict]:
    ...
    try:
        ...
        item_selector = selectors.get("item", "li.search-product")
        try:
            await page.wait_for_selector(item_selector, timeout=10000)
        except PlaywrightTimeout:
            # 대체 셀렉터 시도
            item_selector = "li[class*='search-product']"
            await page.wait_for_selector(item_selector, timeout=8000)
        items = await page.query_selector_all(item_selector)

        for item in items[:5]:  # 상위 5개만
            try:
                name_el = await item.query_selector(selectors.get("name", ".name"))
                price_el = await item.query_selector(selectors.get("price", ".price-value"))
                link_el = await item.query_selector(selectors.get("link", "a"))
                seller_el = await item.query_selector(selectors.get("seller", ".seller"))

                name = await name_el.inner_text() if name_el else ""
                price_text = await price_el.inner_text() if price_el else "0"
                href = await link_el.get_attribute("href") if link_el else ""
                seller = await seller_el.inner_text() if seller_el else "쿠팡"
                ...
                if name and price > 0:
                    products.append({
                        ...
                        "price": price,
                        "shipping_fee": 0,  # 로켓배송 기준
                        "total_price": price,
                        "platform": PLATFORM,
                        "seller": seller.strip(),
                        "is_official": "로켓" in seller or "공식" in name,
                        "url": full_url,
        ...
        print(f"[WARN] 쿠팡 타임아웃: {query}", file=sys.stderr)
    except Exception as e:
        print(f"[WARN] 쿠팡 크롤링 오류 ({query}): {e}", file=sys.stderr)
...
async def crawl(queries: list[dict]) -> list[dict]:
    ...
    async with async_playwright() as p:
        ...
        for q in queries:
            ...
            for term in q.get("search_terms", [])[:2]:  # 최대 2개 검색어
                print(f"[쿠팡] 검색 중: {term}")
...
def main():
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"[WARN] 쿠팡 크롤링 실패 (시도 {attempt}): {e}", file=sys.stderr)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    out_path = OUTPUT_DIR / "raw_coupang_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"platform": PLATFORM, "products": products}, f, ensure_ascii=False, indent=2)

    print(f"[OK] 쿠팡 크롤링 완료: {len(products)}개 상품")
